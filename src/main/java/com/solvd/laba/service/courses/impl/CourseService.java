...
public class CourseService implements ICourseService {
    ...
    public void create(Course course) {
        ...
        if(!course.getOnlineCourses().isEmpty()){
            course.getOnlineCourses().stream().forEach(i -> onlineCourseService.create(i, course.getId()));
        }
        if(!course.getOnsiteCourses().isEmpty()){
            course.getOnsiteCourses().stream().forEach(i->onsiteCourseService.create(i,course.getId()));
        }
        if(!course.getEnrollments().isEmpty()){
            course.getEnrollments().stream().forEach(i->enrollmentService.create(i,course.getId()));
        }
...
courseDAO.delete(id);
    ...
    @Override
    public void getById(Long id) {courseDAO.getById(id);
    }
